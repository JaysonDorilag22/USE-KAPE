...
export default function UserOrder() {
  ...
  useEffect(() => {
    const fetchUserOrders = async () => {
      try {
        const signedInUserId = currentUser._id;
  ...
  useEffect(() => {
    if (selectedStatus === "All") {
      ...
      const filtered = userOrders.filter((order) => order.status === selectedStatus);
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex space-x-4 mb-4 mt-5">
        ...
        <button onClick={() => handleStatusClick("Processing")} className="nav-button">
          Processing
        </button>
        <button onClick={() => handleStatusClick("Delivered")} className="nav-button">
          Delivered
        </button>
        <button onClick={() => handleStatusClick("Cancelled")} className="nav-button">
          Cancelled
        </button>
      </div>
